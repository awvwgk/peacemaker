...
module qce
    ...
    contains
        ...
        subroutine initialize_conserved_quantities()
            ...
            do i = 1, size(monomer)
                global_data%vexcl = global_data%vexcl + &
                    global_data%bxv_pure(i)*global_data%monomer_amounts(i)*clusterset(monomer(i))%volume
            end do
        ...
        subroutine qce_start()
            ...
            ! Start interactive mode if specified in input file.
        ...
        subroutine interactive()
            ...
            integer:: myunit, stat, n, optimizer
            logical:: do_sp
            logical:: stop_im
            
            write(0,*) "Interactive mode"
        ...
        !=================================================================================
        ! Performs a single QCE calculation for a given point x.
        subroutine single_qce(x)
...
!                write(*,'(4(ES13.6,5X),A)') ib%amf*avogadro**2, ib%bxv, ib%error, best_ib%error, "New minimum!"
                best_ib = ib
            end if
...
!                 bxv = 4.92329285056288E-06 * ib%temp(itemp)**2 - &
!                       0.002865742482831 * ib%temp(itemp) + 1.52889991182811
...
!                ib%amf = (0.0 * ib%temp(itemp) + 0.170969)/avogadro**2
!                ib%bxv = 0.000589107142857 * ib%temp(itemp) + 0.863501785714285
                
                amf = ib%amf + ib%temp(itemp) * ib%amf_temp
                bxv = ib%bxv + ib%temp(itemp) * ib%bxv_temp

!                 amf =  7.7391523815122E-07 * ib%temp(itemp)**2 - &
!                        0.00045049807816 * ib%temp(itemp) + 0.250778622026993
!                 amf = amf/avogadro**2
!                 bxv = 4.92329285056288E-06 * ib%temp(itemp)**2 - &
!                       0.002865742482831 * ib%temp(itemp) + 1.52889991182811
        ...
        subroutine calculate_populations(populations, lnq, cyclus, success)
            ...
            integer, dimension(:), allocatable :: number_size
    
            allocate(degree(size(monomer)))
            degree(:) = global_data%degree(:) + 1
            allocate(number_size(99))
            number_size = 0
            ...
            do iclust = 1, size(clusterset)
                associate(c => clusterset(iclust))
                    ...
                    number_size(sum(c%composition)) = number_size(sum(c%composition)) + 1
                end associate
            end do
!            write(*,*) "lnq", lnq%qtot

            ! Calculate coefficients for >> each possible composition <<. We simulate a multi-dimensional
            ! array as a linear or one-dimensional array. n is the number of possible cluster compositions
            ! and thus the number of coefficients in the population polynomial (some of which may be 0).
            ! m is the position of a cluster in the array coeffs_all.
            n = 1
            do i = 1, size(degree)
                n = n * degree(i)
            end do
            allocate(coeffs_all(0:n * size(monomer) -1))
            coeffs_all = 0.0_dp
            do i = 0, size(degree)-1
!                coeffs_all(i*n) = -1.0_dp
                coeffs_all(i*n) = -1.0_dp!real(size(monomer), dp)
            end do

            do iclust = 1, size(clusterset)
               m = clusterset(iclust)%composition(1)
               do j = 2, size(monomer)
                   m = m + clusterset(iclust)%composition(j) * product(degree(:j-1))
               end do
               do i = 1, size(monomer)
 ...
 !                  write(*,*) number_size(sum(clusterset(iclust)%composition)), perm
                   coeffs_all(m + (i-1)*n) = coeffs_all(m + (i-1)*n) + pop_coeffs(i,iclust) !/ &
!                       number_size(sum(clusterset(iclust)%composition)) * perm * size(monomer)
               end do
            end do
...
!            write(*,*) monomer_populations
!            write(*,*) "coeffs", coeffs_all
...
!                monomer_populations = monomer_populations*sum(global_data%ntot)
                monomer_populations = monomer_populations*sum(global_data%ntot)/1.0_dp!size(monomer)
!                write(*,*) monomer_populations
...
!                    populations(i) = exp(tmp)
                    populations(i) = exp(tmp) * 1.0_dp!real(size(monomer), dp)
                end associate
            end do
!            write(*,*) "populations", populations
        ...
        subroutine calculate_volume(vol, vdamp, amf, bxv, temp, populations, success)
            ...
            do i = 1, size(monomer)
                coeffs(0) = coeffs(0) + &
                    global_data%ntot(i)**2*amf_pure_local(i)
                do j = i+1, size(monomer)
                    coeffs(0) = coeffs(0) + &
                        2.0_dp*global_data%ntot(i)*global_data%ntot(j)*amf
                end do
            end do
            coeffs(1) = -coeffs(0)
            coeffs(0) = coeffs(0) * bxv * global_data%vexcl
